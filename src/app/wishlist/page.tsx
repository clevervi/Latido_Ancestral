    ...
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-white to-orange-50 py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center gap-3 mb-8">
          ...
          <h1 className="text-4xl font-bold text-gray-800">Mi Lista de Deseos</h1>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {wishlistProducts.map((product) => (
            ...
            >
              <Link href={`/productos/${product.id}`} className="block relative h-64">
                <Image
                  src={product.image}
                  alt={product.name}
                  fill
        ...
        <div className="mt-8 text-center">
          <p className="text-gray-600 mb-4">
            {wishlistProducts.length} {wishlistProducts.length === 1 ? 'producto' : 'productos'} en tu lista
          </p>
          <Link
            href="/colecciones"
