...
export function generateInvoicePDF(order: Order): void {
  ...
  // Header
  doc.setFontSize(24);
  doc.text('Latido Ancestral', pageWidth / 2, 20, { align: 'center' });
  ...
  doc.text(order.shippingAddress.street, 15, 104);
  doc.text(`${order.shippingAddress.city}, ${order.shippingAddress.state}`, 15, 110);
  doc.text(`${order.shippingAddress.postalCode}, ${order.shippingAddress.country}`, 15, 116);
  ...
  doc.setFontSize(8);
  doc.text('Latido Ancestral - Artesanías Colombianas', pageWidth / 2, footerY, { align: 'center' });
